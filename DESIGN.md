...
1. The content canvas is pitch black. Content surfaces use solid standard materials, primarily `#1C1C1E` and `#242426`.
2. Glass is reserved for navigation, sheets, transient controls, and overlays. Never use glass for ordinary content cards.
3. No decorative gradients, colored fog, ambient orbs, or arbitrary glow effects. Gradients are allowed only when they encode data, such as a chart area or liquid level.
...
10. The tab bar remains visible across top-level views, always shows all four labels, and is hidden only for temporary full-screen camera or modal experiences.
